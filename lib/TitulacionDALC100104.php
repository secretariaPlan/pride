...
{ 
	...
	public static function ObtenServSocialAlumnos($programaId,$inicio=NULL,$fin=NULL)
	{
		...
		$query.= " order by inicio";  //serv_social_alumno.alumnoApellidoP";
		//echo $query;
		$oDb->query($query);
		$arr = array();
		while ($rs = $oDb->getRecord("C"))
	...
	public static function ObtenComitesTutorales($profesorId,$inicio=NULL,$fin=NULL)
	{
		...
		$query = "select comiteTutId,curso_posgrado_tipo.nombre as nivel,
				alumnos,concluido,progPosOtro,posgrado_programa.nombre as programa,inicio,fin
				from comite_tutoral
				join curso_posgrado_tipo on comite_tutoral.nivelId = curso_posgrado_tipo.id
				left join posgrado_programa on comite_tutoral.progPosId = posgrado_programa.id
	...
	{
		$oDb = DbFactory::ObtenDb();
		$query = "select alumno.nombre from titulacion_alumno  
