<script src="<?php echo base_url();?>js/evaluador/formRecHum.js" ></script>
<body>
    <div class="grid flex">
    	<div class="col_12">
            <div class="col_2"></div>
            <div class="col_8">
  				<fieldset>
                    <legend>FORMACI&Oacute;N DE RECURSOS HUMANOS</legend>
                    <table class="striped visible" cellspacing="0" cellpadding="0" >
                        <tr>
                            <td width="50%" rowspan="4">Direcci&oacute;n de t&eacute;sis concluidas</td>
                            <td width="30%">Licenciatura</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Especialidad</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Maest&iacute;a</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Doctorado</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="50%" rowspan="4">Tesis conlcuidas en las que participo como supervisor</td>
                            <td width="30%">Licenciatura</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Especialidad</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Maest&iacute;a</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Doctorado</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="50%" rowspan="4">Ex&acute;menes de titulaci&oacute; en donde particip&oacute; como jurado</td>
                            <td width="30%">Licenciatura</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Especialidad</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Maest&iacute;a</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Doctorado</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="50%" rowspan="3">N&uacute;mero de alumnos asesorados en subprogramas</td>
                            <td width="30%">121</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">127</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">Otros</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="50%">Alumnos con Servicio Social concluido </td>
                            <td width="30%"></td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="50%">Participaci&oacute;n en Comite&eacute;s Totorales</td>
                            <td width="30%"></td>
                            <td width="20%">Valor</td>
                        </tr>
                        <td width="50%" rowspan="2">Tutor&iacute;s de posdoctorales</td>
                            <td width="30%">Concluidas</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                            <td width="30%">En proceso</td>
                            <td width="20%">Valor</td>
                        </tr>
                        <tr>
                                <td width="50%">Otros (precisar)</td>
                                <td width="30%"></td>
                                <td width="20%">Valor</td>
                            </tr>
                    </table>
                    <div class="col_12 center">
                            <button id="botonComentario" class="medium blue">Agregar comentario</button>
                        </div>
                        <div id="comentario" title="Agregar comentario">
                            <div class="col_12">
                                <textarea id="texto" class="col_12" placeholder="Agregue un comentario"></textarea>
                            </div>
                            <div class="12 center">
                                <button class="small green">Guardar</button>
                            </div>
                        </div>
                </fieldset>
            </div>
            <div class="col_2"></div>
        </div>
    </div>
</body>